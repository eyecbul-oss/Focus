...
public class MainActivity extends Activity {
    ...
    private static final String BG = "#F7F6FF";
    private static final String TEXT = "#172033";
    private static final String SUB = "#5F6B7A";
    private static final String BLUE = "#A7C7FF";
    private static final String YELLOW = "#FFE17A";
    private static final String ORANGE = "#FFC29A";
    private static final String GREEN = "#A8E6CF";
    private static final String LILAC = "#D8B4FE";
    private static final String RED = "#FFADAD";
    ...
    @Override protected void onCreate(Bundle b) {
        ...
        LinearLayout root = new LinearLayout(this); root.setOrientation(LinearLayout.VERTICAL); root.setPadding(dp(12), dp(12), dp(12), dp(24)); root.setBackgroundColor(Color.parseColor(BG));
        ...
        navBar = nav(); root.addView(navBar, m(0,8,0,10));
        setContentView(root); show(0);
    }

    private LinearLayout header() { LinearLayout c = glassCard(YELLOW); c.setGravity(Gravity.CENTER); c.setPadding(dp(12), dp(10), dp(12), dp(10)); TextView t = txt("SezR Focus Pro", 24, TEXT, true); t.setGravity(Gravity.CENTER); c.addView(t, wrap()); TextView s = txt("Pastel odak, görev ve sınav takip uygulaması", 12, SUB, false); s.setGravity(Gravity.CENTER); c.addView(s, wrap()); return c; }

    private LinearLayout nav() { LinearLayout n = glassCard("#FFFFFF"); n.setOrientation(LinearLayout.HORIZONTAL); n.setPadding(dp(6), dp(6), dp(6), dp(6)); n.addView(tab("Ana",0,YELLOW), wb()); n.addView(tab("Odak",1,BLUE), wb()); n.addView(tab("Görev",2,GREEN), wb()); n.addView(tab("Sınav",3,ORANGE), wb()); n.addView(tab("Not",4,LILAC), wb()); return n; }
    ...
    private void refreshNav() { if (navBar == null) return; navBar.removeAllViews(); navBar.addView(tab("Ana",0,YELLOW), wb()); navBar.addView(tab("Odak",1,BLUE), wb()); navBar.addView(tab("Görev",2,GREEN), wb()); navBar.addView(tab("Sınav",3,ORANGE), wb()); navBar.addView(tab("Not",4,LILAC), wb()); }
    ...
    private LinearLayout profileCard(){ LinearLayout c=glassCard(BLUE); c.setPadding(dp(16),dp(16),dp(16),dp(16)); c.addView(txt("Profil / Giriş",20,TEXT,true),wrap()); profileText=txt("",15,SUB,false); profileText.setPadding(0,dp(8),0,dp(10)); c.addView(profileText,wrap()); EditText input=input("Adını yaz veya misafir kal"); if(!"Misafir".equals(profile)) input.setText(profile); c.addView(input,h(48,0,0,0,8)); LinearLayout r=row(); Button guest=soft("Misafir"), save=colored("Kaydet",YELLOW); r.addView(guest,wb()); r.addView(save,wb()); c.addView(r,wrap()); guest.setOnClickListener(v->{profile="Misafir";input.setText("");save();updateProfile();}); save.setOnClickListener(v->{String x=input.getText().toString().trim();profile=x.length()==0?"Misafir":x;save();updateProfile();}); return c; }

    private LinearLayout timerCard(){ LinearLayout c=glassCard(BLUE); c.setGravity(Gravity.CENTER); c.setPadding(dp(16),dp(20),dp(16),dp(20)); c.addView(pill("ODAK SAYACI",YELLOW),center()); timer=txt(format(seconds),62,TEXT,true); timer.setGravity(Gravity.CENTER); timer.setPadding(0,dp(16),0,0); c.addView(timer,wrap()); status=txt(running?"Odak seansı çalışıyor":"Hazır",15,SUB,false); status.setGravity(Gravity.CENTER); status.setPadding(0,dp(8),0,dp(16)); c.addView(status,wrap()); LinearLayout modes=row(); modes.addView(mode("25 dk",25),wb()); modes.addView(mode("45 dk",45),wb()); modes.addView(mode("60 dk",60),wb()); c.addView(modes,m(0,0,0,12)); toggle=colored(running?"Duraklat":"Başlat",GREEN); c.addView(toggle,height(52)); Button reset=soft("Sıfırla"), full=colored("Tam Ekran Odak",LILAC); c.addView(reset,h(50,0,8,0,0)); c.addView(full,h(50,0,8,0,0)); toggle.setOnClickListener(v->toggleTimer()); reset.setOnClickListener(v->resetTimer()); full.setOnClickListener(v->fullscreen()); return c; }
    ...
    private LinearLayout glassCard(String accent){ LinearLayout c=new LinearLayout(this); c.setOrientation(LinearLayout.VERTICAL); c.setBackground(bg(accent,26,"#FFFFFF")); return c; }
    ...
    private TextView stat(String value,String label,String color){ LinearLayout box=glassCard(color); box.setPadding(dp(14),dp(14),dp(14),dp(14)); TextView n=txt(value,23,TEXT,true); box.addView(n,wrap()); box.addView(txt(label,12,SUB,false),wrap()); return n; }
    ...
    private LinearLayout.LayoutParams wb(){ LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,dp(46),1); p.setMargins(dp(3),0,dp(3),0); return p; }
