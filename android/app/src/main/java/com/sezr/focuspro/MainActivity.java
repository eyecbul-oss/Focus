...
public class MainActivity extends Activity {
    ...
    private final Runnable tick = new Runnable() {
        @Override public void run() {
            if (!running) return;
            if (seconds > 0) {
                seconds--; updateTimer(); handler.postDelayed(this, 1000);
            } else {
                running = false;
    ...
    private LinearLayout weeklyCard(){ LinearLayout c=card(); c.setPadding(dp(18),dp(18),dp(18),dp(18)); c.addView(txt("Haftalık Özet",22,"#facc15",true),wrap()); weeklyText=txt("",15,"#dbeafe",false); weeklyText.setPadding(0,dp(10),0,0); c.addView(weeklyText,wrap()); return c; }

    private LinearLayout musicCard(){ LinearLayout c=card(); c.setPadding(dp(18),dp(18),dp(18),dp(18)); c.addView(txt("Müzik / Odak Atmosferi",22,"#facc15",true),wrap()); musicText=txt("",15,"#dbeafe",false); musicText.setPadding(0,dp(8),0,dp(12)); c.addView(musicText,wrap()); LinearLayout a=row(), b=row(); a.addView(music("Sessiz"),wb()); a.addView(music("Yağmur"),wb()); a.addView(music("Kütüphane"),wb()); b.addView(music("Beyaz Gürültü"),wb()); b.addView(music("Lo-fi"),wb()); b.addView(music("Doğa"),wb()); c.addView(a,m(0,0,0,8)); c.addView(b,wrap()); return c; }
    ...
    private LinearLayout card(){ LinearLayout c=new LinearLayout(this); c.setOrientation(LinearLayout.VERTICAL); c.setBackground(bg("#111827",24,"#243041")); return c; }
    ...
    private TextView pill(String v){ TextView t=txt(v,13,"#facc15",true); t.setGravity(Gravity.CENTER); t.setPadding(dp(14),dp(8),dp(14),dp(8)); t.setBackground(bg("#241a08",22,"#5f4712")); return t; }
    private Button primary(String v){ Button b=new Button(this); b.setText(v); b.setTextColor(Color.parseColor("#111827")); b.setTextSize(15); b.setTypeface(Typeface.DEFAULT,Typeface.BOLD); b.setBackground(bg("#facc15",16,"#facc15")); return b; }
    private Button dark(String v){ Button b=new Button(this); b.setText(v); b.setTextColor(Color.WHITE); b.setTextSize(14); b.setTypeface(Typeface.DEFAULT,Typeface.BOLD); b.setBackground(bg("#1e293b",16,"#334155")); return b; }
    private TextView stat(String value,String label){ LinearLayout box=card(); box.setPadding(dp(14),dp(14),dp(14),dp(14)); TextView n=txt(value,24,"#facc15",true); box.addView(n,wrap()); box.addView(txt(label,12,"#cbd5e1",false),wrap()); return n; }
