...
public class MainActivity extends Activity {
    ...
    private static final String PINK = "#FFDDEB";
    ...
    private void showFocus() {
        content.addView(title("Odak", "Pomodoro, mola, kategori, ses ve katı mod"), margins(0, 0, 0, 14));
        ...
        content.addView(strictCard(), margins(0, 0, 0, 16));
        updateTimer();
        updateMusic();
    }
    ...
    private void showTasks() {
        ...
        content.addView(tasksCard(), margins(0, 0, 0, 16));
        renderTasks();
    }

    private void showReports() {
        content.addView(title("Rapor", "Günlük, haftalık, kategori ve başarımlar"), margins(0, 0, 0, 14));
        ...
        content.addView(leaderboardCard(), margins(0, 0, 0, 16));
    }

    private void showAi() {
        content.addView(title("AI Rehber", "Yerel koçluk: hedef, sınav ve görev önerisi"), margins(0, 0, 0, 14));
        ...
        content.addView(notesCard(), margins(0, 0, 0, 16));
        updateExam();
        renderNotes();
    }
    ...
    private LinearLayout flowCard() {
        ...
        top.addView(pill(tasks.size() + " görev", BLUE), fixed(100, 42));
        c.addView(top, margins(0, 0, 0, 12));
        if (tasks.isEmpty()) {
            c.addView(text("Bugün görev yok. AI sekmesinden hazır çalışma planı oluşturabilirsin.", 15, SUB, false), wrap());
        } else {
    ...
    private LinearLayout motivationCard() {
        ...
        LinearLayout r = row();
        r.addView(summaryWide("Rozet", badgeCount() + " / 10", YELLOW), weightCell());
    ...
    private LinearLayout profileCard() {
        ...
        r.addView(guest, buttonCell());
        r.addView(saveBtn, buttonCell());
        c.addView(r, height(60));
        guest.setOnClickListener(v -> { profile = "Misafir"; input.setText(""); save(); updateProfile(); });
        saveBtn.setOnClickListener(v -> { String s = input.getText().toString().trim(); profile = s.length() == 0 ? "Misafir" : s; save(); updateProfile(); });
        return c;
    }
    ...
    private LinearLayout timerCard() {
        ...
        c.addView(pill(breakMode ? "MOLA SAYACI" : "ODAK SAYACI", breakMode ? BLUE : YELLOW), center());
        ...
        c.addView(modes, heightMargins(56, 0, 0, 0, 8));
        ...
        c.addView(breaks, heightMargins(56, 0, 0, 0, 12));

        timerButton = button(running ? "Duraklat" : "Başlat", breakMode ? BLUE2 : GREEN2);
        c.addView(timerButton, height(56));
        Button reset = softButton("Sıfırla");
        Button full = button("Tam Ekran Odak", LILAC);
        c.addView(reset, heightMargins(54, 0, 8, 0, 0));
        c.addView(full, heightMargins(54, 0, 8, 0, 0));
    ...
    private LinearLayout categoryCard() {
        ...
        LinearLayout r1 = buttonRow();
        r1.addView(categoryButton("Matematik", GREEN2), buttonCell());
        r1.addView(categoryButton("Problem", ORANGE), buttonCell());
        c.addView(r1, heightMargins(56, 0, 0, 0, 8));
        LinearLayout r2 = buttonRow();
        r2.addView(categoryButton("Okuma", BLUE2), buttonCell());
        r2.addView(categoryButton("Sınav", RED), buttonCell());
        c.addView(r2, height(56));
        return c;
    }
    ...
    private LinearLayout musicCard() {
        ...
        LinearLayout r1 = buttonRow();
        r1.addView(musicButton("Sessiz"), buttonCell());
        r1.addView(musicButton("Yağmur"), buttonCell());
        r1.addView(musicButton("Kütüphane"), buttonCell());
        c.addView(r1, heightMargins(56, 0, 0, 0, 8));
        LinearLayout r2 = buttonRow();
        r2.addView(musicButton("Beyaz Gürültü"), buttonCell());
        r2.addView(musicButton("Lo-fi"), buttonCell());
        r2.addView(musicButton("Doğa"), buttonCell());
        c.addView(r2, height(56));
        return c;
    }
    ...
    private LinearLayout tasksCard() {
        ...
        c.addView(input, heightMargins(52, 0, 12, 0, 8));

        LinearLayout durs = buttonRow();
        durs.addView(durationButton("15 dk", 15), buttonCell());
        durs.addView(durationButton("25 dk", 25), buttonCell());
        durs.addView(durationButton("45 dk", 45), buttonCell());
        c.addView(durs, heightMargins(56, 0, 0, 0, 8));

        LinearLayout templates = buttonRow();
        templates.addView(templateButton("20 Problem", "20 problem çöz", "Problem", 25, BLUE), buttonCell());
        templates.addView(templateButton("Yanlış", "Yanlış analizi yap", "Sınav", 15, RED), buttonCell());
        templates.addView(templateButton("Özet", "Konu özeti çıkar", "Matematik", 10, GREEN), buttonCell());
        c.addView(templates, heightMargins(56, 0, 0, 0, 8));

        Button add = button("Görev Ekle", YELLOW2);
        c.addView(add, height(56));
        taskBox = column();
        taskBox.setPadding(0, dp(12), 0, 0);
        c.addView(taskBox, wrap());
        Button clear = softButton("Tamamlananları Temizle");
        c.addView(clear, heightMargins(54, 0, 10, 0, 0));
    ...
    private LinearLayout reportCard() {
        ...
        r.addView(summaryWide("Ortalama", sessions == 0 ? "0 dk" : (minutes / Math.max(1, sessions)) + " dk", BLUE), weightCell());
        r.addView(summaryWide("Haftalık", weeklyTotal() + " dk", LILAC), weightCell());
        c.addView(r, wrap());
        return c;
    }
    ...
    private LinearLayout achievementsCard() {
        ...
        c.addView(text(badgeCount() + " / 10 başarı açıldı", 16, SUB, false), margins(0, 6, 0, 12));
        ...
        c.addView(achievement("🎯", "Hedefçi", "Günlük hedefi tamamla", minutes >= target, GREEN), margins(0, 0, 0, 8));
        c.addView(achievement("🧠", "AI Planlı", "AI planını görevlere ekle", hasAiTask(), LILAC), wrap());
        return c;
    }
    ...
    private LinearLayout aiPlanCard() {
        ...
        LinearLayout r = buttonRow();
        Button add = button("Planı\nGörevlere Ekle", YELLOW2);
        Button start = button("25 dk\nBaşlat", GREEN2);
        r.addView(add, buttonCell());
        r.addView(start, buttonCell());
        c.addView(r, heightMargins(66, 0, 0, 0, 8));
        LinearLayout quick = buttonRow();
        quick.addView(templateButton("Deneme", "Deneme yanlışlarını incele", "Sınav", 25, RED), buttonCell());
        quick.addView(templateButton("Eksik Konu", "Eksik konu kapat", "Matematik", 25, LILAC), buttonCell());
        quick.addView(templateButton("Okuma", "10 sayfa okuma yap", "Okuma", 20, BLUE), buttonCell());
        c.addView(quick, height(56));
        add.setOnClickListener(v -> { addAiPlanTasks(); show(2); });
        start.setOnClickListener(v -> { breakMode = false; focusDuration = 25 * 60; seconds = focusDuration; focusCategory = "Matematik"; show(1); });
        return c;
    }
    ...
    private LinearLayout examCard() {
        ...
        EditText n = input("Sınav adı"); n.setText(examName); c.addView(n, heightMargins(52, 0, 10, 0, 8));
        EditText d = input("yyyy-MM-dd"); d.setText(examDate); c.addView(d, heightMargins(52, 0, 0, 0, 8));
        Button saveExam = button("Sınavı Kaydet", YELLOW2);
        c.addView(saveExam, height(56));
    ...
    private LinearLayout notesCard() {
        ...
        c.addView(add, height(56));
        c.addView(clear, heightMargins(54, 0, 8, 0, 0));
    ...
    private LinearLayout settingsCard() {
        ...
        c.addView(text("Günlük hedef", 15, TEXT, true), margins(0, 4, 0, 8));
    ...
    private void toggleTimer() {
        ...
        if (running) {
            ...
            if (strictMode && !breakMode) strictDialog();
            if (timerButton != null) timerButton.setText("Duraklat");
            if (statusText != null) statusText.setText(breakMode ? "Mola çalışıyor" : "Odak seansı çalışıyor");
            handler.post(tick);
        } else {
            ambient.stop();
    ...
    private void showFullscreen() {
        ...
        LinearLayout c = card();
        c.setGravity(Gravity.CENTER);
        TextView title = text(breakMode ? "Mola" : focusCategory, 26, TEXT, true);
        ...
        fullStatus = text(running ? (breakMode ? "Mola çalışıyor" : "Odak seansı çalışıyor") : "Başlamak için hazır", 18, SUB, false);
        ...
        fullButton = button(running ? "Duraklat" : "Başlat", breakMode ? BLUE2 : GREEN2);
        Button close = softButton("Çık");
        c.addView(fullButton, height(60));
        c.addView(close, heightMargins(60, 0, 12, 0, 0));
    ...
    private void showCompleteDialog(int earned) {
        String title = breakMode ? "Mola Tamamlandı" : "Seans Tamamlandı";
        String msg = breakMode ? "Molan bitti. Şimdi kısa bir odak seansına dönebilirsin." : earned + " dk odak süresi eklendi. AI rehber raporunu güncelledi.";
        simpleDialog(title, msg, breakMode ? "Odaklan" : "Tamam", breakMode ? BLUE2 : YELLOW2).show();
    }

    private void strictDialog() {
        simpleDialog("Katı Mod Başladı", "Telefonu çevirmeden, uygulamadan çıkmadan ve bildirimlere bakmadan seansı tamamlamaya çalış.", "Odaklan", GREEN2).show();
    }
    ...
    private Dialog simpleDialog(String title, String message, String ok, String color) {
        ...
        Button b = button(ok, color); c.addView(b, height(56));
    ...
    private void updateAll() {
        ...
        updateMusic();
        updateExam();
        if (settingsText != null) settingsText.setText("Hedef: " + target + " dk\nMola: " + breakDuration + " dk\nSeans sesi: " + (soundOn ? "Açık" : "Kapalı") + "\nKatı mod: " + (strictMode ? "Açık" : "Kapalı"));
    }

    private void updateTimer() { if (timerText != null) timerText.setText(format(seconds)); syncFull(); }
    private void syncFull() { if (fullTimer != null) fullTimer.setText(format(seconds)); if (fullStatus != null) fullStatus.setText(running ? (breakMode ? "Mola çalışıyor" : "Odak seansı çalışıyor") : "Duraklatıldı"); if (fullButton != null) fullButton.setText(running ? "Duraklat" : "Devam Et"); }
    ...
    private Button modeButton(String label, int min) { Button b = softButton(label); b.setOnClickListener(v -> { if (running) return; breakMode = false; focusDuration = min * 60; seconds = focusDuration; updateTimer(); if (statusText != null) statusText.setText(label + " odak modu seçildi"); }); return b; }
    private Button breakButton(String label, int min) { Button b = button(label, breakMode && breakDuration == min ? BLUE2 : BLUE); b.setTextSize(10); b.setOnClickListener(v -> { if (running) return; breakMode = true; breakDuration = min; focusDuration = min * 60; seconds = focusDuration; save(); show(1); }); return b; }
    private Button categoryButton(String label, String color) { Button b = button(label.equals(focusCategory) ? "✓ " + label : label, color); b.setOnClickListener(v -> { focusCategory = label; breakMode = false; save(); show(1); }); return b; }
    ...
    private Button targetButton(String label, int value) { Button b = button(target == value ? "✓ " + label : label, target == value ? GREEN2 : "#FFFFFF"); b.setOnClickListener(v -> { target = value; save(); show(0); }); return b; }
    private Button templateButton(String label, String text, String category, int min, String color) { Button b = button(label, color); b.setTextSize(9); b.setOnClickListener(v -> { tasks.add(new FocusTask(text, false, category, min)); save(); simpleDialog("Görev Eklendi", text + " görev listene eklendi.", "Tamam", color).show(); }); return b; }

    private void addAiPlanTasks() {
        int d = daysToExamInt();
        if (d <= 30) {
            tasks.add(new FocusTask("AI Plan: Deneme yanlışlarını analiz et", false, "Sınav", 25));
            tasks.add(new FocusTask("AI Plan: Eksik konudan 20 soru çöz", false, "Problem", 25));
            tasks.add(new FocusTask("AI Plan: Yanlış defterine 5 madde yaz", false, "Sınav", 10));
        } else if (minutes < target / 2) {
            ...
            tasks.add(new FocusTask("AI Plan: Kısa tekrar yap", false, "Matematik", 15));
            tasks.add(new FocusTask("AI Plan: Zor soruları işaretle", false, "Problem", 15));
        }
        save();
    }

    private String aiPlanText() {
        if (daysToExamInt() <= 30) return "1) 25 dk deneme yanlış analizi\n2) 25 dk eksik konudan soru çözümü\n3) 10 dk yanlış defteri\n4) " + breakDuration + " dk mola\n5) Gün sonu rapor kontrolü";
        if (minutes < target / 2) return "1) 10 dk konu özeti\n2) 25 dk soru çözümü\n3) 5 dk yanlış analizi\n4) " + breakDuration + " dk mola\n5) 25 dk tekrar seansı";
        return "1) 15 dk kısa tekrar\n2) 15 dk zor soru işaretleme\n3) " + breakDuration + " dk mola\n4) Rapor ve rozet kontrolü";
    }
    ...
    private String aiAdvice() {
        ...
        else s.append("Görev listen boş; AI planını görevlere ekle. ");
    ...
    private boolean hasAiTask() { for (FocusTask t : tasks) if (t.text.startsWith("AI Plan")) return true; return false; }
    private int badgeCount() { int c = 0; if (sessions >= 1) c++; if (sessions >= 5) c++; if (minutes >= 60) c++; if (minutes >= 200) c++; if (progress() >= 100) c++; if (activeDayCount() >= 3) c++; if (tasks.size() > 0) c++; if (doneTaskCount() >= 3) c++; if (hasAiTask()) c++; if (weeklyTotal() >= 300) c++; return c; }
    ...
    private void save() {
        ...
        SharedPreferences.Editor e = prefs.edit()
            .putInt("sessions", sessions).putInt("minutes", minutes).putInt("target", target).putInt("break_duration", breakDuration)
    ...
    private void clearAll() {
        ...
        minutes = 0; sessions = 0; target = 60; breakDuration = 5; seconds = focusDuration; profile = "Misafir"; musicMode = "Sessiz"; strictMode = false; breakMode = false;
    ...
    private void refreshNav() { if (navBar != null) { navBar.removeAllViews(); addTabs(navBar); } }
    private void addTabs(LinearLayout n) { n.addView(tab("⌂\nPano", 0, YELLOW2), navCell()); n.addView(tab("◷\nOdak", 1, BLUE2), navCell()); n.addView(tab("✓\nGörev", 2, GREEN2), navCell()); n.addView(tab("▥\nRapor", 3, ORANGE), navCell()); n.addView(tab("✦\nAI", 4, LILAC), navCell()); }
    ...
    private LinearLayout chartRow(String label, int value, int max, String color) { LinearLayout r = column(); r.addView(text(label + "  " + value + " dk", 14, TEXT, true), wrap()); int pct = Math.min(100, Math.round(value * 100f / max)); r.addView(progressBar(pct, color), heightMargins(14, 0, 4, 0, 0)); return r; }
    private LinearLayout achievement(String icon, String title, String desc, boolean done, String color) { LinearLayout r = row(); r.setGravity(Gravity.CENTER_VERTICAL); r.setPadding(dp(12), dp(10), dp(12), dp(10)); r.setBackground(bg(done ? color : "#F8FAFF", 18, "#E5E7EF")); r.addView(text(icon, 28, TEXT, true), fixed(48, 54)); LinearLayout t = column(); t.addView(text(title, 17, TEXT, true), wrap()); t.addView(text(desc, 13, SUB, false), wrap()); r.addView(t, new LinearLayout.LayoutParams(0, -2, 1)); r.addView(text(done ? "✓" : "○", 24, done ? "#0F9F6E" : SUB, true), fixed(34, 54)); return r; }
    private LinearLayout leaderRow(String rank, String name, String score, String color) { LinearLayout r = row(); r.setGravity(Gravity.CENTER_VERTICAL); r.setPadding(dp(12), dp(12), dp(12), dp(12)); r.setBackground(bg(color, 18, "#FFFFFF")); r.addView(text(rank, 20, TEXT, true), fixed(54, 52)); r.addView(text(name, 17, TEXT, true), new LinearLayout.LayoutParams(0, -2, 1)); r.addView(text(score, 17, TEXT, true), fixed(90, 52)); return r; }

    private LinearLayout card() { LinearLayout c = column(); c.setPadding(dp(16), dp(18), dp(16), dp(20)); c.setBackground(bg("#FFFFFF", 28, "#E5E7EF")); return c; }
    private LinearLayout column() { LinearLayout l = new LinearLayout(this); l.setOrientation(LinearLayout.VERTICAL); return l; }
    private LinearLayout row() { LinearLayout l = new LinearLayout(this); l.setOrientation(LinearLayout.HORIZONTAL); return l; }
    private LinearLayout buttonRow() { LinearLayout l = row(); l.setGravity(Gravity.CENTER_VERTICAL); return l; }
    ...
    private Button button(String label, String color) { Button b = new Button(this); b.setText(label); b.setTextColor(Color.parseColor(TEXT)); b.setTextSize(12); b.setTypeface(Typeface.DEFAULT, Typeface.BOLD); b.setGravity(Gravity.CENTER); b.setAllCaps(false); b.setIncludeFontPadding(false); b.setMinHeight(0); b.setMinimumHeight(0); b.setPadding(dp(4), 0, dp(4), 0); b.setBackground(bg(color, 18, "#FFFFFF")); return b; }
    ...
    private LinearLayout.LayoutParams fixed(int w, int h) { return new LinearLayout.LayoutParams(dp(w), dp(h)); }
    ...
    private LinearLayout.LayoutParams buttonCell() { LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, -1, 1); p.setMargins(dp(4), 0, dp(4), 0); return p; }
    private LinearLayout.LayoutParams navCell() { LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, dp(50), 1); p.setMargins(dp(3), 0, dp(3), 0); return p; }
