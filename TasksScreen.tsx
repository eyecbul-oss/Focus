...
export const useFocusStore = create<FocusState>((set, get) => ({
  ...
  addTask: (title, type = 'Soru', priority = 'normal') =>
    set((s) => ({
      tasks: [{ id: Date.now().toString(), title, type, priority, done: false, createdAt: new Date().toISOString() }, ...s.tasks]
    })),
  ...
  noteToTask: (id) =>
    set((s) => {
      ...
      if (!note) return s;
      return {
        tasks: [{ id: Date.now().toString(), title: 'Nottan görev: ' + note.text, type: 'Yanlış', priority: 'normal', done: false, createdAt: new Date().toISOString() }, ...s.tasks],
      };
    }),
  ...
  finishPomodoro: (minutes = 25, mode = 'Standart') =>
    set((s) => ({
      ...
      totalToday: s.totalToday + minutes * 60,
